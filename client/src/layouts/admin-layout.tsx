...
const mainNav: NavItem[] = [
  { to: paths.admin.dashboard, label: 'Tổng quan', icon: LayoutDashboard, end: true },
    ...
    <div className="flex min-h-screen bg-background">
      <aside className="flex min-h-screen w-60 shrink-0 flex-col border-r border-border/70 bg-[var(--nc-sidebar)]">
        <div className="shrink-0 border-b border-border/50 p-5">
          <p className="font-display text-xs tracking-[0.2em] text-primary uppercase">NihongoCoach</p>
        ...
        <nav className="min-h-0 flex-1 space-y-0.5 overflow-y-auto p-3">
          {mainNav.map(({ to, label, icon: Icon, end }) => (
            <NavLink key={to} to={to} end={end} className={({ isActive }) => sidebarLinkClass(isActive)}>
        ...
        <div className="mt-auto shrink-0 space-y-0.5 border-t border-border/50 p-3">
          <NavLink to={paths.learn.hub} className={({ isActive }) => sidebarLinkClass(isActive)}>
            <GraduationCap className="size-4 shrink-0" />
            <span>Chế độ học (test học viên)</span>
          </NavLink>
          <button type="button" onClick={handleLogout} className={sidebarLinkClass(false)}>
